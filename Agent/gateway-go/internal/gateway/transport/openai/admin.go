...
func (s *Server) handleAdminAgentLayers(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		resp, err := s.admin.AgentLayers()
		if err != nil {
			writeError(w, http.StatusBadGateway, "admin_error", err.Error())
			return
		}
	...
	case http.MethodPost:
		var req admin.UpdateAgentLayersRequest
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			writeError(w, http.StatusBadRequest, "invalid_request", err.Error())
			return
		}
		resp, err := s.admin.UpdateAgentLayers(req)
		if err != nil {
			writeError(w, http.StatusBadGateway, "admin_error", err.Error())
			return
		}
